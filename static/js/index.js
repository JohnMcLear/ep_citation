...
exports.aceDrop = function (hook, citation) {
  ...
  const div = $(citation.e.target).closest('div');
  const jsdiv = citation.e.target;
  ...
  if (!emptyLine) {
    ...
    const offsetX = citation.e.originalEvent.clientX;
    const offsetY = citation.e.originalEvent.clientY;
    const divOffsetTop = $(citation.e.target).closest('div').offset().top;

    // We can compute how far down the target line the mouse event is..
    const lineDifference = offsetY - divOffsetTop;
    ...
    if ('getComputedStyle' in window) {
      ...
      if (cs.length !== 0) for (let i = 0; i < cs.length; i++) styles.push([cs.item(i), cs.getPropertyValue(cs.item(i))]);
      // Opera workaround. Opera doesn't support `item`/`length`
      // on CSSStyleDeclaration.
      //
      else for (var k in cs) if (cs.hasOwnProperty(k)) styles.push([k, cs[k]]);

    // The IE way
    //
    } else if ('currentStyle' in element) {
      var cs = element.currentStyle;
      for (var k in cs) styles.push([k, cs[k]]);
    }
    // End of borrwed code
    ...
    const oldWorker = $('iframe[name="ace_outer"]').contents().find('#outerdocbody').find('#citationWorker');
    $(oldWorker).remove();
    ...
    const newLine = `<span style='position:absolute;top:0;left:0;z-index:999999' id='citationWorker' class='ghettoCursorXPos'>${divHTML}</span>`;

    // Add the HTML to the DOM
    $('iframe[name="ace_outer"]').contents().find('#outerdocbody').append(newLine);

    // Get the worker element
    const worker = $('iframe[name="ace_outer"]').contents().find('#outerdocbody').find('#citationWorker');
    ...
    var selStart = 0;
    $.each(leftOffsets, (key, spanOffset) => {
  ...
  const typeId = citation.e.originalEvent.dataTransfer.types.length - 1;
  const type = citation.e.originalEvent.dataTransfer.types[typeId];
  var citation = JSON.parse(citation.e.originalEvent.dataTransfer.getData(type));
  const cleanId = citation.miscId.replace(/[^a-z0-9]/gi, '');
  ...
  padeditor.ace.callWithAce((ace) => {
    ...
    // console.log(sel);
    const citationNumber = countProperties(clientVars.ep_citation.citations) + 1;
    if (!citation.quote) {
    ...
    if (citation.quote) {
      ...
      const quoteLength = citation.quote.length;
...
exports.aceAttribsToClasses = function (hook, citation) {
  ...
  if (citation.key == 'citation') {
    ...
    const id = citation.value.split('$$')[0];
    const title = citation.value.split('$$')[1];
...
exports.aceInitialized = function (hook, citation) {
  const editorInfo = citation.editorInfo;
  clientVars.ep_citation = {};
  clientVars.ep_citation.citations = {};
};
...
exports.aceCreateDomLine = function (hook_name, args, cb) {
  if (args.cls.indexOf('citation:') >= 0) {
    ...
    const title = clientVars.ep_citation.citations[value].title;
